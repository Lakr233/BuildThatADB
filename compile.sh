...
for ARCH in $ARCHS_LIST; do
    ...
    TEMP_SRC_DIR_WITH_ARCH=$TEMP_SRC_DIR/$ARCH
    echo "[+] copying source to temp dir: $TEMP_SRC_DIR_WITH_ARCH"
    mkdir -p $TEMP_SRC_DIR_WITH_ARCH || true
    echo "[*] transferring..."
    pushd $SOURCE_DIR >/dev/null
    tar cf - * | (
        cd $TEMP_SRC_DIR_WITH_ARCH
        tar xf -
    )
    ...
    pushd $TEMP_SRC_DIR_WITH_ARCH >/dev/null
    ...
    for source in $SOURCE_LIST; do
        ...
        if [ -f $BUILD_SCRIPT_PATH ]; then
            ...
            pushd $TEMP_SRC_DIR_WITH_ARCH/$name >/dev/null
            $BUILD_SCRIPT_PATH
            popd >/dev/null
        else
